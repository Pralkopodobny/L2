...
def first_epoch(network: Net.Network, training_set, alpha=0.6):
    ...
    for l, layer in enumerate(network.layers):
        ...
        squared_previous_changes.append(change * change)
    return squared_previous_gradients, squared_previous_changes


def process_mini_batch(network: Net.Network, training_set, squared_previous_gradients, squared_previous_changes, number_of_updates:int, alpha=0.6):
    mini_bath_errors = []
    mini_bath_gradients = []
    for data, label in training_set:
        network.process_input(data)
    ...
    for l, layer in enumerate(network.layers):
        mean_gradients_in_batch_in_layer = sum([el[l] for el in mini_bath_gradients]) / len(training_set)
        change = np.sqrt((squared_previous_changes[l] / number_of_updates) + epsilon) / np.sqrt((squared_previous_gradients[l] / number_of_updates) + epsilon) * mean_gradients_in_batch_in_layer
        layer.weights = layer.weights - change
...
def epoch(network, training_set, batch_size, mini_batches_count, squared_previous_gradients, squared_previous_changes, number_of_updates:int, alpha=0.6):
    index = 0
    last_index = min(batch_size, len(training_set))
    for i in range(mini_batches_count):
        number_of_updates = number_of_updates + 1
        process_mini_batch(network, training_set[index:last_index], squared_previous_gradients, squared_previous_changes, number_of_updates, alpha)
        index = last_index
        last_index = min(last_index + batch_size, len(training_set))
    return number_of_updates


def train_network_adadelta(network: Net.Network, training_set, validation_set, batch_size, iterations=8, alpha=0.6):
    mini_batches_count = -(len(training_set) // -batch_size)
    errors = []
    squared_previous_gradients, squared_previous_changes = first_epoch(network, training_set)
    number_of_updates = 1

    for i in range(iterations):
        number_of_updates = epoch(network, training_set, batch_size, mini_batches_count, squared_previous_gradients, squared_previous_changes, number_of_updates, alpha)
